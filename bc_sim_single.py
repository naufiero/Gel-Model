...
cytod_surf = meshio.read(path + "ellipse" + ".msh")
cytod_faces = cytod_surf.cells[0].data
cytod_vol = meshio.read(path + "ellipse_vol_r4" + ".msh")

mesh = Mesh()
with XDMFFile(path + "ellipse_vol_r4" + ".xdmf") as infile:
    infile.read(mesh)
...
vert_disp = pd.read_csv(path + "disps.csv",
                        header=None).values
